...
N = 40        # N个汽车，M个服务器，
M = 10
anm = np.zeros((1,N*M))                  # 初始化自变量,含有N*M个元素,0.5为严格可行点
...
def opt(x0, k):
    ...
    obj2.append(Pr(xe.T))     # 每次迭代后整数解进行局部交换对应最大值加入列表
    variance(xe.T)
...
def visual(x,y):
    ...
    # 设置子图
    plt.subplot(111)
    plt.title("指数分布下内点法", fontsize=25, backgroundcolor='#3c7f99',
...
if __name__ == '__main__':
    ...
    np.savetxt('data/50/IPMA-decimal-obj.txt', obj1, fmt="%.4f")
    #np.savetxt('data/20/IPMA-Integer-obj.txt', obj2, fmt="%.4f")
    np.savetxt('data/50/IPMA-Integer-obj.txt', obj2, fmt="%.4f")
    visual(n,obj1)